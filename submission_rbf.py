...
def get_bernoulli_model(input_matrix, vocabulary):
    # arguments: 
    #   input_matrix = [ input_list = [word]]
    #   based on word_table
    # return:
...
def get_feature(input_matrix, vocabulary, do_bern, do_log, do_norm):
    ...
    # using frequency model or bernoulli model
    if do_bern:
        output_matrix = get_bernoulli_model(input_matrix, vocabulary)
    else:
...
def get_x_train(strategy_instance, vocabulary, do_bern, do_log, do_norm):
    word_matrix = strategy_instance.class0 + strategy_instance.class1
    feature_matrix = get_feature(word_matrix, vocabulary, do_bern, do_log, do_norm)
...
def get_prediction(clf, feature_file_path, vocabulary, do_bern, do_log, do_norm):
...
def log_result(clf, vocabulary, do_bern, do_log, do_norm):
    dim = len(vocabulary)
    ...
    with open('./rbf_nonorm_bern.txt', 'a+') as handle:
        handle.write('######################################################\n')
